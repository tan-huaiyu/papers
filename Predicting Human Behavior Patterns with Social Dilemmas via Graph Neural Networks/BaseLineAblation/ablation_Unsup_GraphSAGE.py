...
class GraphSAGE(torch.nn.Module):
    def __init__(self, in_size, hid_size, out_size):
        super().__init__()
        self.conv1 = pyg_nn.SAGEConv(in_size, hid_size)
        self.conv2 = pyg_nn.SAGEConv(hid_size, hid_size)
...
if __name__ == "__main__":
    ...
    gnn_dataset_t_lst = [
        Pre_Data.Nets(N_num, 4).graph_data_set(
            "single", g, Y[:, i].reshape((N_num, 1)), margin_data1[:, i]
        ) for i in range(L)
    ]

    dataset_fix = gnn_dataset_t_lst[int(L*Evo_size)]
    input_size = dataset_fix.num_node_features
    model = GraphSAGE(input_size, hidden_size, output_size)
    ...
    pd.DataFrame(evo_pre).to_csv("C:/Users/killspeeder/Desktop/abl_SAGE_evo.csv", index=False)
    train_pre = {'ACC': ACC, 'F1': F1, 'TPR': TPR, 'FPR': FPR}
    pd.DataFrame(train_pre).to_csv("C:/Users/killspeeder/Desktop/abl_SAGE_train.csv", index=False)
