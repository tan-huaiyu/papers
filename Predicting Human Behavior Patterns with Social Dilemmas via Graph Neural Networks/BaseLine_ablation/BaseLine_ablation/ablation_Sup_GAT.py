...
class GAT(torch.nn.Module):
    def __init__(self, in_size, hid_size, out_size):
        super().__init__()
        self.conv1 = pyg_nn.GATConv(in_size, hid_size)
        self.conv2 = pyg_nn.GATConv(hid_size, hid_size)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(evo_pre).to_csv("C:/Users/killspeeder/Desktop/abl_GAT_evo.csv", index=False)
    train_pre = {'ACC': ACC, 'F1': F1, 'TPR': TPR, 'FPR': FPR}
    pd.DataFrame(train_pre).to_csv("C:/Users/killspeeder/Desktop/abl_GAT_train.csv", index=False)
