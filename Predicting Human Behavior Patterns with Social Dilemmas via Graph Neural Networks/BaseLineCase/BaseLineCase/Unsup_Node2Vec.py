...
class Node2Vec:
    ...
    def sample_strategy(self, v, t):
        ...
        if len(neb_v) == 0:
            return False
        weights = self.Margin_Payoffs[neb_v]  # with attributes
        for i, x in enumerate(neb_v):
            if t == x:  # go back
                weights[i] *= 1/self.p
            elif not self.G.has_edge(t, x):  # go forward
                weights[i] *= 1/self.q
        weights = np.exp(weights) / np.sum(np.exp(weights), axis=0)  # softmax for stochastic row
...
if __name__ == "__main__":
    ...
    pd.DataFrame(evo_pre).to_csv("C:/Users/killspeeder/Desktop/node2vec_evo.csv", index=False)
    train_pre = {'ACC': ACC, 'F1': F1, 'TPR': TPR, 'FPR': FPR}
    pd.DataFrame(train_pre).to_csv("C:/Users/killspeeder/Desktop/node2vec_train.csv", index=False)
